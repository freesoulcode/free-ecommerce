...
func (r *ProductRepository) ListAdminProducts(ctx context.Context, query domainproduct.ListAdminProductsQuery) ([]*domainproduct.Summary, int64, error) {
	whereClause, args := buildAdminProductWhere(query)
...
SELECT
    ...
    p.sub_title,
    p.main_image_url,
    COALESCE(MIN(s.price_amount), 0) AS min_price_amount,
    COALESCE(MAX(s.price_amount), 0) AS max_price_amount,
    COALESCE(MIN(s.currency), '') AS currency,
    COALESCE(SUM(s.stock), 0) AS total_stock,
    p.review_status,
    p.sale_status,
    p.created_at,
...
LEFT JOIN product_skus s ON s.product_id = p.id
` + whereClause + `
GROUP BY p.id, p.shop_id, p.shop_name, p.title, p.sub_title, p.main_image_url, p.review_status, p.sale_status, p.created_at, p.updated_at
ORDER BY p.updated_at DESC, p.id DESC
LIMIT ? OFFSET ?`
...
func (r *ProductRepository) GetAdminProduct(ctx context.Context, id int64) (*domainproduct.Detail, error) {
	var model ProductModel
	if err := r.db.WithContext(ctx).Where("id = ?", id).First(&model).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, appErrors.NotFound("product not found")
		}
		return nil, fmt.Errorf("find admin product: %w", err)
	}

	var skuModels []ProductSKUModel
	if err := r.db.WithContext(ctx).Where("product_id = ?", id).Order("price_amount ASC, id ASC").Find(&skuModels).Error; err != nil {
		return nil, fmt.Errorf("list admin product skus: %w", err)
	}
	...
	if len(skuModels) > 0 {
		detail.Currency = skuModels[0].Currency
	}
	for i, sku := range skuModels {
		detail.SKUs = append(detail.SKUs, &domainproduct.SKU{
			ID:          sku.ID,
...
func (r *ProductRepository) ReviewProduct(ctx context.Context, id int64, reviewStatus string) (*domainproduct.Detail, error) {
	updates := map[string]any{"review_status": reviewStatus, "updated_at": time.Now().UTC()}
	result := r.db.WithContext(ctx).Model(&ProductModel{}).Where("id = ?", id).Updates(updates)
	if result.Error != nil {
		return nil, fmt.Errorf("review product: %w", result.Error)
	}
	if result.RowsAffected == 0 {
		return nil, appErrors.NotFound("product not found")
	}
	return r.GetAdminProduct(ctx, id)
}
...
func buildAdminProductWhere(query domainproduct.ListAdminProductsQuery) (string, []any) {
	...
	if reviewStatus := strings.TrimSpace(query.ReviewStatus); reviewStatus != "" {
		clauses = append(clauses, "AND p.review_status = ?")
		args = append(args, reviewStatus)
	}
	if saleStatus := strings.TrimSpace(query.SaleStatus); saleStatus != "" {
		clauses = append(clauses, "AND p.sale_status = ?")
